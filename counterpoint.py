...
def write_par_file(wl_start, wl_stop, datadir, prefix, index, zeeman_prefix):
    outfile_name = os.path.join(datadir, zeeman_prefix)+'/'+prefix+str(index)+'.par'
    ...
    if stronglines:
        pf.write('stronglines_in      \'./'+prefix+'strong_linelist_'+str(index)+'.out\'\n')
        pf.write('strong          1\n')
    ...
    for T in temps:
        ...
        for G in gravs:
            ...
            pf.write('smoothed_out   \'./output/'+prefix+'MARCS_T'+str(T)+'G'+str(G)+'_'+str(index)+'.moog\'\n')
...
def generateLineList(datadir, prefix, wl_start, wl_stop, Bfield):
    ...
    for line in open(strong_file, 'r').readlines():
        ...
        if ( (float(l[0]) > wl_start) & (float(l[0]) < wl_stop) ):
            ...
            gup = float(l[7])
            VdW = float(l[9])
            strongLines.append(MOOGTools.spectral_Line(wave, species, exp_pot, loggf, Jlow=Jlow, Jup=Jup, glow=glow,
            gup=gup, VdW=VdW))
    ...
    for zp in zeeman_polarization:
        ...
        sfn = os.path.join(datadir,zp)+'/'+prefix+'strong_linelist_'+str(strong_file_counter)+'.out'
        outfile = open(sfn, 'w')
        ...
        while i < len(strongLines):
            if (strong_line_counter + len(strongLines[i].zeeman[zp][0])) < strong_line_limit:
                strongLines[i].dump(out=outfile, zeeman=zp, mode='MOOG')
                strong_line_counter += len(strongLines[i].zeeman[zp][0])
                i = i + 1
            else:
                breakpoints.append(numpy.mean([strongLines[i].wl, strongLines[i-1].wl]))
                j = i
                while ( (strong_line_counter < strong_line_limit) & (j < len(strongLines) ) ):
                ...
                wfn = os.path.join(datadir,zp)+'/'+prefix+'weak_linelist_'+str(strong_file_counter)+'.out'
                weakfile = open(wfn, 'w')
                bm = scipy.where( (weakLines > breakpoints[-2]) & (weakLines < breakpoints[-1]) )[0]
                if len(bm) == 0:
                    weakfile.write('%10.3f%10s%10.3f%10.3f\n' % (numpy.mean([breakpoints[-2], breakpoints[-1]]), 26.0, 6.0, -8.0))
                for k in bm:
                ...
                write_par_file(breakpoints[-2], breakpoints[-1], datadir, prefix, strong_file_counter, zp)
        ...
        wfn = os.path.join(datadir,zp)+'/'+prefix+'weak_linelist_'+str(strong_file_counter)+'.out'
        weakfile = open(wfn, 'w')
        #bm = scipy.where( (weakLines > breakpoints[-2]) & (weakLines < breakpoints[-2]+(breakpoints[-1]-breakpoints[-2])) )[0]
...
for B in [0.0, 2.0]:
    for feature in zip(prefixes, wl_starts, wl_stops):
        ...
        generateLineList(datadir, prefix, wl_start*10000.0, wl_stop*10000.0, B/10.0)
